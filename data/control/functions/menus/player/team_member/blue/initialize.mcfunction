#> control:menus/player/team_member/blue/initialize
#
...
data modify storage control:menu Initialize set value {tag:{CtrlEnderChest:true,JumpTo:"Player.Blue-TeamMember"},Count:1b}

## アイコンとなるアイテムを指定
data modify storage control:menu Initialize.id set value "minecraft:blue_wool"

## メニューの名前(JP)を指定
data modify storage control:menu Initialize.tag.display.Name set value '{"text":"青チームのメンバー","color":"blue"}'

## 説明文を指定(任意)
data modify storage control:menu Initialize.tag.display.Lore set value ['{"text":"メンバーの一覧","color":"white","italic":false}','{"text":""}']

## 表示するスロット番号を指定 byte型
data modify storage control:menu Initialize.Slot set value 19b
