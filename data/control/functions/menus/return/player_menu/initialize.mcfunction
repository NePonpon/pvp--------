#> control:menus/return/player_menu/initialize
#
...
data modify storage control:menu Initialize set value {tag:{CtrlEnderChest:true,JumpTo:"Return.PlayerMenu"},Count:1b}

## アイコンとなるアイテムを指定
data modify storage control:menu Initialize.id set value "minecraft:iron_door"

## メニューの名前(JP)を指定
data modify storage control:menu Initialize.tag.display.Name set value '{"text":"プレイヤー設定画面にもどる","color":"dark_gray","bold":true}'
