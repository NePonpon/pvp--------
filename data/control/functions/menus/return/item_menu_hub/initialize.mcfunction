#> control:menus/return/item_menu_hub/initialize
#
...
data modify storage control:menu Initialize set value {tag:{CtrlEnderChest:true,JumpTo:"Return.ItemMenuHub"},Count:1b}

## アイコンとなるアイテムを指定
data modify storage control:menu Initialize.id set value "minecraft:iron_door"

## メニューの名前(JP)を指定
data modify storage control:menu Initialize.tag.display.Name set value '{"text":"アイテム一覧メニュー画面にもどる","color":"dark_gray","bold":true}'
