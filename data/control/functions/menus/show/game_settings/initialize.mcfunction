...
data modify storage control:menu Initialize set value {tag:{CtrlEnderChest:true,JumpTo:"Show.GameSettings"},Count:1b}

## アイコンとなるアイテムを指定
data modify storage control:menu Initialize.id set value "minecraft:command_block"

## メニューの名前(JP)を指定
data modify storage control:menu Initialize.tag.display.Name set value '{"text":"ゲーム設定メニュー","color":"yellow","bold":true}'
