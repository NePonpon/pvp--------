#> control:menus/item_menu/hub/magics/initialize
#
...
data modify storage control:menu Initialize set value {tag:{CtrlEnderChest:true,JumpTo:"ItemMenu.Hub.Magics"},Count:1b}

## アイコンとなるアイテムを指定
data modify storage control:menu Initialize.id set value "minecraft:carrot_on_a_stick"

## メニューの名前(JP)を指定
data modify storage control:menu Initialize.tag.display.Name set value '{"text":"攻撃＆補助魔法ページ","bold":true,"color":"gray"}'
